...
def test_gateway_cds_generation(proxy, expected_config):
    ...
    actual_cluster_config = create_cds_response([Service('foo')], {'test': [Service('bar')]}, proxy)
...
def test_gateway_rds_generation(proxy, expected_config):
    ...
    actual_routes_config = create_rds_response([Service('foo')], {'test': [Service('bar')]}, proxy, domain='hail.is')
